...
pub async fn generate_commit_message(
    ...
    let diff_trimmed = if diff.len() > 10_000 {
        format!("{}... [truncated]", &diff[..10_000])
    } else {
        diff
    };
...
pub async fn explain_git_changes(
    ...
    match kind.as_str() {
        ...
        "working" => {
            // Staged-only — matches commit-message scope and stays fast.
            let diff = git::git_staged_diff(project_path.clone()).unwrap_or_default();
            if diff.trim().is_empty() {
                return Err(AppError::Message(
        ...
        "conflict" => {
            ...
            for f in conflicted.iter().take(12) {
                ...
                let body = std::fs::read_to_string(&full).unwrap_or_else(|_| {
                    "(could not read file — binary or missing)".into()
                });
...
pub async fn summarize_issue(
    ...
    if issue.get("pull_request").is_some() {
        return Err(AppError::Message(
            "This is a pull request — open it under Pull requests to summarize.".into(),
        ));
    }

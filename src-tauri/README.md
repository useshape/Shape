...
| `src/commands/` | Implementation plus `commands/ipc/` (Tauri `#[tauri::command]` handlers) and `commands/preview/` |
| `src/domain/` | Async wrappers around git / filesystem / packages / terminal / outline |
| `src/core/` | Shared paths, process helpers, Windows toasts |
| `src/mcp/` | MCP client, OAuth, credentials |
| `runtime/` | Inlined into HTML/design preview (`bundle.js`, `tailwind-browser.js`). Built from `shape/runtime/entry.ts` via `npm run build:runtime`. |

...
export function insertCssVariableInContent(content: string, name: string, value: string): string {
    const nextName = normalizeVariableName(name);
    if (!nextName || !value.trim()) return content;
    if (parseCssVariables(content).some((item) => item.name === nextName)) {
        return updateCssVariableInContent(content, nextName, value.trim());
    }
    const decl = `  ${nextName}: ${value.trim()};`;
    const root = content.match(/:root(?:\s*,\s*:host)?\s*\{/);
    if (root && root.index != null) {
        const open = content.indexOf("{", root.index);
        if (open >= 0) {
            return `${content.slice(0, open + 1)}\n${decl}${content.slice(open + 1)}`;
        }
    }

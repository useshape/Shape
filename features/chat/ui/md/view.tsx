...
function createMarkdownComponents(options?: { nested?: boolean; isGenerating?: boolean; isLast?: boolean }) {
    ...
    return {
        ...
        code(props: { className?: string; children?: React.ReactNode }) {
                ...
                return (
                    <div className="my-4 rounded-lg overflow-hidden border border-border-subtle bg-panel">
                        <div className="flex items-center justify-between px-4 py-1.5 bg-panel border-b border-border-subtle">
                            <span className="text-[0.92em] font-medium text-text-muted">{match[1]}</span>
                        </div>
                        <SyntaxHighlighter
            ...
            if (isBlock) {
                return (
                    <pre className="my-3 overflow-x-auto rounded-lg border border-border-subtle bg-panel p-3 text-[0.92em] font-mono leading-relaxed">
            ...
            if (isFilePath) {
                ...
                return (
                    <span
                        className="inline-flex items-center gap-1.5 text-[0.92em] bg-panel hover:bg-panel-hover px-2 py-0.5 rounded-lg border border-border-subtle text-text-primary cursor-pointer transition-colors mx-0.5 align-middle font-sans"
            ...
            return (
                <code
                    className="rounded border border-border-subtle bg-panel px-1.5 py-0.5 text-[0.92em] font-sans text-text-primary"
                    {...rest}
                >
                    {children}
                </code>
        ...
        p: ({ children }: { children?: React.ReactNode }) => (
            <p className="mb-2 last:mb-0 font-sans font-medium text-text-primary">
                {children}
            </p>
        ),
        ...
        h1: ({ children }: { children?: React.ReactNode }) => (
            <h1 className="font-medium mt-4 mb-2 text-text-primary font-sans">{children}</h1>
        ),
        h2: ({ children }: { children?: React.ReactNode }) => (
            <h2 className="font-medium mt-3 mb-1.5 text-text-primary font-sans">{children}</h2>
        ),
        h3: ({ children }: { children?: React.ReactNode }) => (
            <h3 className="font-medium mt-2 mb-1 text-text-primary font-sans">{children}</h3>
        ),
...
export const ChatMarkdown = React.memo(({
    ...
    return (
        <div
            className="font-sans chat-markdown-body"
        >
            <ReactMarkdown

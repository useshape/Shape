...
/** Unified line diff without Monaco. */
export function SimpleDiffView({
    ...
    const rows = useMemo(() => {
        ...
        for (const part of parts) {
            ...
            // diffLines keeps a trailing empty when value ends with \n
            const cleaned =
        ...
        <div className="h-full min-h-0 overflow-auto bg-editor font-mono text-sm custom-scrollbar">
            <div className="min-w-full py-2">
                {rows.map((row, i) => (
                    ...
                    >
                        <span className="w-10 shrink-0 select-none px-1 text-right text-text-muted tabular-nums">
                            {row.oldNo ?? ""}
                        </span>
                        <span className="w-10 shrink-0 select-none px-1 text-right text-text-muted tabular-nums">
                            {row.newNo ?? ""}

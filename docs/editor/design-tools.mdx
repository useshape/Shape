...
Shape includes design-aware tooling for web UI work. Completions, inline controls, and diagnostics help you edit Tailwind classes and CSS variables without leaving the editor.
...
Toggle scrubbing, underlines, cursor-bind, and wheel-on-inputs in **Settings → Design**.

## Design blame

Hover a UI token to see a plain-language “last changed” blurb from git blame. Disable via **Settings → Design → Design Blame Hover**.
...
Enable or disable this in **Settings → Design → Design Autocomplete**.
...
Configure severity in **Settings → Design → Design Diagnostics**. Choose **AA** or **AAA** contrast level.
...
The chat panel includes a **Visual** mode for UI work. Ask it to preview a component in chat before adding it, or tell it to build without stopping. See [Chat Modes](/docs/ai/chat-modes).

...
- Click the **commit** icon on the [activity bar](/docs/workspace/titlebar-and-panels)
...
Open [Git Manager](/docs/git/git-manager) from the Shape logo in the graph header (or **Window → Git Manager**) for a larger graph plus PRs and checks.
...
Expand a commit row to see files changed in that commit. Author avatars can be toggled in [Git Settings](/docs/settings/git-settings).
